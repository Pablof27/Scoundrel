--[[
    ButtonView
    A clickable UI button. Receives assets via constructor — no globals.
]]

local Button = {}
Button.__index = Button

function Button:new(params, assets, mouseProvider)
    ...
    o.disabled  = params.disabled or false
    o.assets    = assets
    o.mouseProvider = mouseProvider  -- function that returns mouseX, mouseY
...
function Button:render()
    ...
    local luminosity, alpha, textAlpha
    if self.disabled then
        luminosity = 0.5
        alpha = 0.4
        textAlpha = 0.5
    else
        luminosity = self.hoovered and 1.0 or 0.9
        alpha = self.color[4]
        textAlpha = 1
    end

    love.graphics.setColor(self.color[1] * luminosity, self.color[2] * luminosity, self.color[3] * luminosity, alpha)
    ...
    love.graphics.setColor(0.9, 0.9, 0.9, textAlpha)

    local font = self.assets.fonts.medium
